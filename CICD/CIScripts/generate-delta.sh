...
if [[ "$ENVIRONMENT" =~ SF-QA|SF-UAT|SF-Release ]]; then
  ...
  CMDT_QUERY_RESULT=$(sf data query "SELECT Value__c FROM DeployInfo__mdt WHERE Key__c = 'LastDeploySHA'" \
    --target-org "$ORG_ALIAS" --json || true)

  DEPLOYED_SHA=$(echo "$CMDT_QUERY_RESULT" | jq -r '.result.records[0].Value__c')
